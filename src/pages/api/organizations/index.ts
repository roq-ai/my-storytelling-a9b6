...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.expert_story?.length > 0) {
      const create_expert_story = body.expert_story;
      body.expert_story = {
        create: create_expert_story,
      };
    } else {
      delete body.expert_story;
    }
    if (body?.sales_story?.length > 0) {
      const create_sales_story = body.sales_story;
      body.sales_story = {
        create: create_sales_story,
      };
    } else {
      delete body.sales_story;
    }
